...
| 版本 | 变更 |
|---|---|
| R1.8.0 | `status` 补上 Admin Token / API Key / 面板与 API 地址（v4+v6）显示——此前只报服务状态，用户无从查看凭据。新增菜单项「查看凭据」与「轮换 API Key」及对应子命令 `creds` / `rotate`（轮换直接改 systemd `Environment=` 并重启+自测）。`install.sh` 结束时提示后续查看凭据的方式 |
