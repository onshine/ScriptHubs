...
- [workbuddy2api-manager](workbuddy2api-manager/) — workbuddy2api 网关 + 管理面板部署套件。R1.0.1，`workbuddy2api.sh` 一键部署/更新/自检（旧名 `deploy.sh` 保留为兼容外壳），`NET_MODE=host` 应对 LXC 里 Docker bridge 出网不通，含端口链/登录链路/容器出网三重自检与面板密码重置。
