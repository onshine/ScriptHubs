...
| 端口链（容器内 → 宿主 → HTTP 200） | 三段分开验，坏在哪一段一目了然；**端口按模式自动选**（详见下文） |
| 登录链路 | 拿密码真的 POST 一次 `/api/auth/login` |
| **上游可达性** | **逐个探真实的 LLM 上游**，告诉你哪家能用哪家不能用 |
| **监听地址** | **host 模式下最关键的一项**：实测有没有暴露到非回环地址 |
| 网络模式 | 当前记录的是 bridge 还是 host |

### 关于「上游可达性」这项

⚠️ **不要拿"随便一个大站"当出网探针** —— 这是个真实的教训。最初这版探的是
`api.github.com`（失败再退到 `www.baidu.com`），结果在一台**能连 OpenAI/Anthropic、
但连不上 github 和百度**的机器上报了假警报，让用户白排查一轮。

现在的探针取自 9Router 自己的 provider registry（`open-sse/providers/registry/*.js`
里的 `baseUrl`），逐个探这些：
...
**判定标准：任何 HTTP 状态码都算「通」。** 403 / 404 / 421 只说明服务器正常应答了，
TCP + TLS 链路是完整的；只有「连不上 / 超时 / DNS 失败」才算不通。
（`wget` 对 4xx 也会返回非 0，所以判据是看有没有握手痕迹，不是看退出码。）

输出长这样：

```
── 上游可达性自检 ──
  网关上游可达性: 4 个可达 / 2 个不可达
      ❌ 不可达：Google Gemini DeepSeek
[!]      部分上游不可达。9Router 只能转发【可达】的那几家，
[!]      配置上游时请优先选上面 ✅ 可达的（连不通的连了也只会一直报错）。
```

**部分不可达是正常现象** —— 很多机器的出站策略只放行部分目标。这时按提示
避开不可达的那几家就行。全部不可达时脚本会自动再跑一次 DNS 诊断，
直接告诉你问题在 DNS 还是在路由/防火墙（省得来回猜）。

### 端口链的坑：容器内端口随模式变

`status` 里「容器内」那一段探的端口**随网络模式变化**：

- bridge → 容器内 `20128`（`CTR_PORT`），靠端口映射对外
- host → 容器内就是 `15900`（`PORT`），共用网络栈

R1.1.0 之前这里硬编码了 `CTR_PORT`，导致 host 模式下探 20128（那上面什么都没有），
报出假的「容器内不通」—— 而同一屏里「监听地址」和「登录链路」又都是通的，
很容易让人以为服务有问题。现在由 `_ctr_port()` 按模式返回正确端口。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| R1.1.1 | 2026-09-22 | 修两个自检误报。① 「上游可达性」原来拿 `api.github.com` / `www.baidu.com` 当出网探针，在一台能连 OpenAI/Anthropic 但连不上 github/百度的机器上必然误报；改为逐个探真实的 LLM 上游域名（取自 9Router 自身的 provider registry），且**任何 HTTP 状态码都算通**，只有连不上才算不通，输出逐上游可达性表。② host 模式下端口链探错端口（探容器内 20128，实际是 15900），报假的「容器内不通」；新增 `_ctr_port()` 按模式返回正确端口。另加 DNS / 路由 自动区分诊断。 |
