#!/usr/bin/env bash
# workbuddy2api + workbuddy-manager 部署 / 更新脚本 R1.0.0
#
...
#
# 用法：
#   ./deploy.sh                      # 部署或更新（自动停旧实例再起新的）
#   ./deploy.sh stop                 # 只停止两个容器
#   ./deploy.sh restart              # 只重启两个容器
#   ./deploy.sh status               # 看状态与端口链路
#   ./deploy.sh logs                 # 跟踪日志
set -euo pipefail
...
reset_panel_password() {
  local newpwd="$1"
  [[ -f "$USERS_FILE" ]] || die "找不到 $USERS_FILE（面板还没跑过？先执行 ./deploy.sh）"
...
case "$CMD" in
  ...
  status)
    ...
    if [[ -f "$USERS_FILE" ]]; then
      ...
      echo "  忘记密码：./deploy.sh reset-password"
    else
...
# ============ 以下是部署主流程 ============
[[ $EUID -eq 0 ]] || die "请用 root 运行（或 sudo ./deploy.sh）"
...
for p in "$GW_PORT" "$PANEL_PORT"; do
  ...
  ■ 若是【别的服务】占着，换个端口重跑即可：
        GW_PORT=27863 PANEL_PORT=27864 ./deploy.sh
...
if [[ "$PWD_STATE" == "exists" ]]; then
  if [[ -n "$PANEL_PASSWORD" ]]; then
    ...
    warn "  如需改密码：./deploy.sh reset-password '新密码'"
  fi
  PANEL_PASSWORD=""
...
cat <<EOF
  ...
  ⚠️ 忘了密码就用这个重置（会吊销所有已登录会话）：
        ./deploy.sh reset-password '你的新密码'
     或不带参数让它生成随机密码：
...
============================================================
  ...
  日常命令：
    ./deploy.sh status             状态 + 端口链 + 登录链路自检
    ./deploy.sh logs               跟踪日志
    ./deploy.sh restart            重启两个容器
    ./deploy.sh stop               停止两个容器
    ./deploy.sh reset-password     重置面板密码
